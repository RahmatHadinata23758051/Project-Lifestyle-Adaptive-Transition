...
def rebalance_daily_budget(
    ...
    Ensures zero divide protection and negative budget safety.
    """
    if remaining_days <= 0:
        return {
            "remaining_budget": round(weekly_budget - total_spent_so_far, 2),
            "new_daily_budget_cap": 0.0,
            "is_overbudget": total_spent_so_far > weekly_budget,
            "overbudget_amount": max(0.0, total_spent_so_far - weekly_budget),
        }
